...
export const getWeatherForecast = async (latitude: number, longitude: number): Promise<Forecast[]> => {
  try {
    ...
    const response = await axios.get(
      `https://weatherapp-backend-frankfurt.onrender.com/api/weather/forecast?latitude=${latitude}&longitude=${longitude}`
    );
...
export const getWeatherSummary = async (latitude: number, longitude: number): Promise<Summary> => {
  try {
    // Make a GET request to backend API
    const response = await axios.get(
      `https://weatherapp-backend-frankfurt.onrender.com/api/weather/summary?latitude=${latitude}&longitude=${longitude}`
    );
